...
- Every committed packaged journey initializes `shell-finalization.json` before launch. Each after-exit sanitation attempt, including no-session launch-failure and retry paths, appends a fail-closed record with route/action, process-proof context, success disposition or failure, candidate path, Jump stage transition, and exact build identity. The recorder refuses to run sanitation without exact process absence and records a durable blocked entry when process proof or S1 prerequisites are unavailable. A sidecar write failure is itself a finalization failure and preserves diagnostic artifacts.
