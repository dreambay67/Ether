...
| --- | --- | --- | --- | --- | --- | --- |
| T01-ledger | Luna | xhigh | Build the 250-item machine-readable acceptance ledger, explicit RX baseline classification, deterministic validator, status report, and owner-route mappings. No product behavior edits. | `1e6879f..1d78ef2` | 250/250 source records; revised baseline counts are FAIL 19, MISSING 9, PRESENT-UNPROVEN 222; historical 14 remain PRESENT-UNPROVEN; `pnpm.cmd run evidence:validate` and `pnpm.cmd run evidence:validate:self-test` pass; candidate/gate checks enforce hashes, canonical journeys, and fixture indicators. Main review’s RX triage and journey-contract findings are resolved. | Accepted by main worker after the review corrections in `1d78ef2`; Phase 0 independent review remains separate and pending. |
