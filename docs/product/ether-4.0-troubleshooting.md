...
| The document is read-only | Another writer, a network/cloud restriction, future format, recoverable corruption, or a native location probe that could not classify the volume within 12 seconds may be responsible. Inspect/export safely; retry after the volume is responsive, close the other writer, or Repair to a new document. | Copy a lock/recovery file beside the document; Ether owns this state in AppData. |
