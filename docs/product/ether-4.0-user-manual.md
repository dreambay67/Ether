...
Antigravity profiles are **Nano Banana 2**, **Nano Banana Pro**, and **Nano Banana 2 Lite**. Nano Banana 2 is the minimum release route; Pro and Lite remain disabled unless their probes pass. Lite is restricted to verified 1K output. Ether records the requested profile independently from provider-reported identity. Authentication, capability, invalid input, timeout, cancellation, process, and malformed-output failures are distinct. No subscription CLI operation silently becomes a paid API call.
