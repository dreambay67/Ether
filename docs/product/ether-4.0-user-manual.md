...
| **Antigravity** | official `agy.exe`, isolated per-attempt staging, verified requested profile | disabled until executable/authentication/conformance are valid |

Antigravity profiles are **Nano Banana 2**, **Nano Banana Pro**, and **Nano Banana 2 Lite**. All three passed the Ether 4.0 release probe against Antigravity CLI 1.1.7. Its image tool accepts aspect ratio but has no resolution parameter. Real 2K and 4K requests on Nano Banana 2 and Pro still returned 1K files, so Ether offers only the exact conformed 1K pixel size for each verified ratio. It never labels that output 2K or 4K. Lite is also restricted to verified 1K output. Changing ratio selects the matching verified pixel dimensions automatically. Ether reuses the machine's persisted Antigravity session silently; an ordinary image run does not open a browser or authentication window. Ether records the requested profile independently from provider-reported identity. A changed CLI version or executable hash disables stale capabilities until a new probe passes. Authentication, capability, invalid input, timeout, cancellation, process, and malformed-output failures are distinct. No subscription CLI operation silently becomes a paid API call.
...
3. In **Concurrent run**, keep sequential execution or select up to eight total calls. Ether applies the lower Codex, Antigravity, or unknown-provider cap and injects each item's dimension values into its effective prompt.
