...
export function EtherEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, data }: EdgeProps & { data: EtherFlowEdgeData }) {
  ...
  return <><g onMouseEnter={revealLabel} onMouseLeave={concealLabelSoon}><path d={path} fill="none" stroke="transparent" strokeWidth={hitStrokeWidth} className="ether-edge-hit-target" data-lane-index={data.laneIndex} data-lane-count={data.laneCount} onClick={(event) => { event.stopPropagation(); data.onSelect(id); }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); if (!data.readOnly) data.onDelete(id); }} /><BaseEdge id={id} path={path} markerEnd={markerEnd} style={{ stroke: `var(--ether-channel-${source})`, strokeWidth: data.selected ? 3 : 2 }} /></g>
