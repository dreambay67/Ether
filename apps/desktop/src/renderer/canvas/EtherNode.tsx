...
export type EtherCanvasNodeData = { node: SchemaNode; connectedInput: PayloadChannel[]; connectedOutput: PayloadChannel[]; readOnly: boolean; onSelect(id: string, additive: boolean): void; onDelete(id: string): void; onResize(id: string, size: { width: number; height: number }): void; onTitle(id: string, title: string): void; };
...
export const EtherNode = memo(function EtherNode({ id, data, selected }: NodeProps & { data: EtherCanvasNodeData }) {
  ...
  const summary = node.config.kind === "prompt.text" ? node.config.body : node.config.kind === "prompt.worker" ? node.config.instruction : node.config.kind === "canvas.note" ? node.config.body : node.config.kind === "generation.image" ? `${node.config.aspectRatio} - ${node.config.outputCount} output${node.config.outputCount === 1 ? "" : "s"}` : subtitle;
  return <article className={`ether-node ether-node-family-${family}${selected ? " is-selected" : ""}`} data-testid="ether-node" data-node-family={family} onClick={(event) => data.onSelect(id, event.ctrlKey || event.metaKey || event.shiftKey)}>
