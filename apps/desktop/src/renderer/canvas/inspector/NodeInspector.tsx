...
function ProviderFields({ context }: { context: InspectorNodeContext }) {
  ...
  useEffect(() => { void app()?.query(globalQuery("provider.capabilities", {})).then((response) => setCapabilities((response.payload?.capabilities as ProviderCapability[] | undefined) ?? [])).catch((error: unknown) => report(error instanceof Error ? error.message : "Provider capabilities are unavailable.")); }, [report]);
  ...
  return <><InspectorSection title="Provider" help="Model profile, aspect ratio, resolution, and limits are supplied by the selected verified capability.">
    ...
    {draft.draft.kind === "generation.image" ? <><label>Aspect ratio <Help label="Aspect ratio" text={controlHelp.aspect} /><select aria-label="Aspect ratio" value={draft.draft.aspectRatio} onChange={(event) => draft.update((current) => current.kind === "generation.image" ? { ...current, aspectRatio: event.target.value } : current)}>{selected.aspectRatios.map((value) => <option key={value}>{value}</option>)}</select></label><label>Resolution <Help label="Resolution" text={controlHelp.resolution} /><select aria-label="Resolution" value={`${draft.draft.resolution.width}x${draft.draft.resolution.height}`} onChange={(event) => { const resolution = selected.resolutions.find((item) => `${item.width}x${item.height}` === event.target.value); if (resolution) draft.update((current) => current.kind === "generation.image" ? { ...current, resolution: { width: resolution.width, height: resolution.height } } : current); }}>{selected.resolutions.map((resolution) => <option key={resolution.id} value={`${resolution.width}x${resolution.height}`}>{resolution.label}</option>)}</select></label></> : null}
