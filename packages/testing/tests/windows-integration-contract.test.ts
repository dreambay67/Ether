...
describe("A02 Windows integration harness contracts", () => {
  ...
  it("keeps user-realistic Explorer focus proofs adjacent to native Enter and drag", () => {
    ...
    const associationForeground = associationScript.indexOf("A02E:after-foreground", chromeClick);
    const associationSelection = associationScript.indexOf("A 'Enter'", associationForeground);
    const associationSetFocus = associationScript.indexOf("$item.SetFocus()", associationSelection);
    const associationStabilityLoop = associationScript.indexOf("for($ss=0;$ss -lt 2;$ss++)", associationSetFocus);
    const associationStableSample = associationScript.indexOf("A 'E'", associationStabilityLoop);
    const associationStableSelection = associationScript.indexOf("$sc=$wr.FindAll", associationStabilityLoop);
    const associationStableRuntime = associationScript.indexOf("GetRuntimeId())) -ne $ir", associationStabilityLoop);
    const associationStableFocus = associationScript.indexOf("FocusedElement", associationStabilityLoop);
    const associationStableExplorer = associationScript.indexOf("Enter:explorer-foreground", associationStabilityLoop);
    const associationMinimized = associationScript.indexOf("Enter:minimized", associationStabilityLoop);
    const associationStableEtherPid = associationScript.indexOf("Enter:stability", associationMinimized);
    const associationInputAbi = associationScript.indexOf("Enter:INPUT ABI", associationStableEtherPid);
    const associationStarted = associationScript.indexOf("$as = [DateTime]::UtcNow", associationInputAbi);
    const associationEnter = associationScript.indexOf("[EtherA02Native]::SendInput(2,$ei,$iz", associationStarted);
    const associationKeyStatePoll = associationScript.indexOf("ReturnReleased()", associationEnter);
    const associationReturnState = associationScript.indexOf("A02E:return-state", associationEnter);
    const associationTransition = associationScript.indexOf("Enter:transition", associationReturnState);
    ...
    expect(associationSetFocus).toBeGreaterThan(associationSelection);
    expect(associationStabilityLoop).toBeGreaterThan(associationSetFocus);
    expect(associationStableSample).toBeGreaterThan(associationStabilityLoop);
    expect(associationStableSelection).toBeGreaterThan(associationStabilityLoop);
    expect(associationStableRuntime).toBeGreaterThan(associationStableSelection);
    expect(associationStableFocus).toBeGreaterThan(associationStableRuntime);
    expect(associationStableExplorer).toBeGreaterThan(associationStableFocus);
    expect(associationMinimized).toBeGreaterThan(associationStableExplorer);
    expect(associationStableEtherPid).toBeGreaterThan(associationMinimized);
    expect(associationInputAbi).toBeGreaterThan(associationStableEtherPid);
    expect(associationStarted).toBeGreaterThan(associationInputAbi);
    expect(associationEnter).toBeGreaterThan(associationStarted);
    expect(associationKeyStatePoll).toBeGreaterThan(associationEnter);
    expect(associationReturnState).toBeGreaterThan(associationEnter);
    expect(associationReturnState).toBeGreaterThan(associationKeyStatePoll);
    expect(associationTransition).toBeGreaterThan(associationReturnState);
    expect(associationScript).toContain("[EtherA02Native]::SendInput(2,$ei,$iz");
    expect(associationScript).toContain("if ($in -ne 2)");
    expect(associationScript).toContain("GetLastWin32Error");
    expect(associationScript).toContain("Enter:cleanup SendInput inserted");
    ...
    expect(associationScript).toContain("$si.Current.HasKeyboardFocus");
    ...
    expect(associationScript).not.toContain("keybd_event");
    expect(associationScript).toContain("if ($rd) { $ri = [EtherA02Native]::EnterRelease()");
    expect(associationScript).toContain("inserted=' + $in + ' returnReleased=' + $rr + ' samples=' + $ss");
    expect(associationScript).not.toContain("[EtherA02Pointer]::SendInput");
    expect(associationScript).toContain("ClientToScreen($xh,[ref]$co)");
    expect(associationScript).toContain("$cx -ge $co.X");
    expect(associationScript).toContain("A02E:chrome-frame");
    ...
    expect(associationScript).toContain("if($cu.X -ne $cx -or $cu.Y -ne $cy){throw 'A02E:cursor-mismatch'}");
    expect(associationScript).toContain("A02E:cursor-hit-root");
  ...
  it("proves association and drag focus transitions inside their exact Explorer interactions", async () => {
    ...
    expect(associationRoute).toContain("waitForFocusedAssociationDiagnostics");
    expect(integrationSpec).toContain("candidatePathHash");
    expect(integrationSpec).toContain("candidateBasenameHash");
    expect(integrationSpec).toContain("canonicalPathHash");
    expect(integrationSpec).toContain("canonicalBasenameHash");
    expect(integrationSpec).toContain("primaryProcessId !== input.primaryPid");
    expect(integrationSpec).toContain("handled.disposition !== \"focused\"");
    expect(integrationSpec).toContain("postSettleFocused !== true");
    expect(integrationSpec).toContain("postSettleMinimized !== false");
    expect(integrationSpec).toContain("postSettleVisible !== true");
    expect(integrationSpec).toContain("events=received>focus-attempt>handled(focused)>focus-state");
