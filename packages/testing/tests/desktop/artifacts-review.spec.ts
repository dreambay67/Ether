    ...
    Object.defineProperty(window, "ether", { value: {
      ...
      application: { onEvent: () => () => undefined,
        query: async (query: { name: string; payload: Record<string, unknown> }) => {
          ...
          if (query.name === "review.checkpoints") return { name: query.name, payload: { checkpoints: [{ id: "checkpoint-1", planId: "plan-1", stepId: "step-1", workItemId: null, state: "waiting-review", selectedOutputVersionIds: [], candidateOutputVersionIds: ["output-00000", "output-00001"], selectionMode: "one", minimumSelections: 1, completion: null, createdAt: "2026-07-22T00:00:00.000Z", completedAt: null }], nextCursor: null } };
