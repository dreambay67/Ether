...
function buildNativeFolderDialogScript(ownerPid: number, directoryPath: string): string {
  return [
    ...
    "  $matches = @([System.Windows.Automation.AutomationElement]::RootElement.FindAll([System.Windows.Automation.TreeScope]::Children, $byPid) | Where-Object { $_.Current.ClassName -eq '#32770' -and $_.Current.Name -eq $dialogTitle -and $_.Current.NativeWindowHandle -ne 0 -and -not $_.Current.IsOffscreen })",
    "  if ($matches.Count -gt 1) { throw ('Expected at most one exact Ether-owned folder dialog; found ' + $matches.Count) }",
    "  if ($matches.Count -eq 1 -and -not [EtherT20FolderDialog]::IsWindowEnabled($ownerHwnd)) { $dialog = $matches[0] } else { Start-Sleep -Milliseconds 150 }",
    "}",
    "if ($null -eq $dialog) { throw 'Exact same-process modal Choose Export Folder dialog did not appear within 15 seconds.' }",
    ...
    "$confirm = @($descendants | Where-Object { $_.Current.ControlType -eq [System.Windows.Automation.ControlType]::Button -and $_.Current.AutomationId -eq '1' -and -not $_.Current.IsOffscreen -and $_.Current.IsEnabled })",
    "if ($confirm.Count -ne 1) { throw ('Exact Ether-owned folder dialog exposed ' + $confirm.Count + ' visible standard confirmation controls after navigation') }",
    "$invoke = $null",
