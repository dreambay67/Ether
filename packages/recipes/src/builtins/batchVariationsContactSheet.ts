...
export const batchVariationsContactSheetRecipe = manifest({
  ...
  graph: graph(graphRef, "Batch Variations and Contact Sheet", [prompt, batch, image, join, compare, collection], [edge("prompt-image", "prompt", "image", "text"), edge("batch-image", "batch", "image", "data"), { ...edge("image-contact-sheet", "image", "contact-sheet", "image"), selector: { kind: "all" } }, { ...edge("contact-sheet-compare", "contact-sheet", "compare", "image"), selector: { kind: "all" } }, edge("compare-collection", "compare", "collection", "image")]),
  requirements: [generate], checkpoints: [checkpoint(graphRef, "compare", "Approve batch selects")], calls: 6, workItems: 9,
