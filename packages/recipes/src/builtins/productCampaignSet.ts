...
const join = node("campaign-set", "flow.join", "Assemble campaign set", 888, 82, { kind: "flow.join", strategy: "ordered", requireComplete: true });
const compare = node("compare", "review.compare", "Select campaign set", 1166, 82, { kind: "review.compare", selectionMode: "many", minimumSelections: 2 });
const collection = node("collection", "output.collection", "Campaign selects", 1444, 82, { kind: "output.collection", collectionId: "campaign-selects", membershipMode: "add", makePrimary: true });
...
export const productCampaignSetRecipe = manifest({
  ...
  graph: graph(graphRef, "Product Campaign Set", [references, prompt, batch, image, join, compare, collection], [edge("product-image", "product", "image", "image", "product"), edge("prompt-image", "prompt", "image", "text"), edge("formats-image", "formats", "image", "data"), { ...edge("image-campaign-set", "image", "campaign-set", "image"), selector: { kind: "all" } }, { ...edge("campaign-set-compare", "campaign-set", "compare", "image"), selector: { kind: "all" } }, edge("compare-collection", "compare", "collection", "image")], "product"),
  requirements: [generate], checkpoints: [checkpoint(graphRef, "compare", "Approve the campaign set")], calls: 4, workItems: 7,
