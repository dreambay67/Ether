...
const filter = node("filter", "review.filter", "Route decisions", 888, 82, { kind: "review.filter", match: "all", rules: [{ id: "approved", field: "decision", operator: "eq", value: "pass" }], routes: [{ id: "selects", label: "Selects", outcome: "matched" }, { id: "rework", label: "Needs rework", outcome: "unmatched" }] });
...
export const evaluateAndRouteRecipe = manifest({
  ...
  graph: graph(graphRef, "Evaluate and Route", [inputs, compare, evaluate, filter, selects, rework], [
    ...
    { ...edge("filter-selects", "filter", "selects", "image"), selector: { kind: "all" } },
    { ...edge("filter-rework", "filter", "rework", "image", "negative"), selector: { kind: "all" } }
  ], "inputs"),
  requirements: [llm], checkpoints: [checkpoint(graphRef, "compare", "Confirm the candidate set")], calls: 1, workItems: 5,
  scenario: { id: "evaluate-and-route-fake", steps: [{ kind: "success", requirementId: "evaluate", latencyMs: 8, outputs: [{ graphRef, nodeRef: "evaluate", channel: "data", fixtureId: "evaluation-data", mediaType: "application/json" }, { graphRef, nodeRef: "evaluate", channel: "image", fixtureId: "evaluation-candidate", mediaType: "image/png" }] }] }
});
