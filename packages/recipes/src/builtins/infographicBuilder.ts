...
const worker = node("structure", "prompt.worker", "Plan hierarchy", 332, 82, { kind: "prompt.worker", behavior: "extract", instruction: "Turn the source brief into a concise factual hierarchy, labels, and visual priorities.", profile: "balanced", model: "gpt-5", reasoningEffort: "medium", variation: 0.1, reviewPolicy: "inspect-first", contextPolicy: { includeUpstream: true, includeDownstreamCapabilities: true, maxTokens: 3000 }, memoryPolicy: { mode: "stateless" }, outputContract: { channel: "text", count: 1, selectionPolicy: "latest" } });
